...
describe("Order repository test", () => {
  ...
  it("should update an order", async () => {
    ...
    const orderToUpdate = new Order("123", "123", [ordemItem]);;

    await orderRepository.update(orderToUpdate)
    try{
      const orderUpdated = await orderRepository.find(orderToUpdate.id);

      expect(orderUpdated).toStrictEqual({
        id: "123",
        customer_id: "123",
        total: order.total(),
        items: [
  ...
  it("should throw an error when order is not found", async () => {
    ...
    }).rejects.toThrow("Order with id 456ABC not found");
  });
